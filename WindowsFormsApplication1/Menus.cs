...
namespace WFHelper
{
    public partial class MainForm
    {
        ...
        private void contextMenuStrip1_Opening(object sender, CancelEventArgs e)
        {
            ...
            File.Copy(Application.StartupPath + "\\" + "implementation0.log.0", _fl.LogsDirectory + "AdditionalLog.log",true);

            ContextMenu2.Items.Add("AdditionalLog");
        }
        ...
        private void HubListUpdate(string name)
        {
            if (_companyList == null) return;

            if (_companyList.Contains(name))
            {
                _companyList.Remove(name);
                _companyList.Add(name);
            }
            else
            {
                _companyList.Add(name);
